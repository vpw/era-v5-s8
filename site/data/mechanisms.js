...
{n:8, id:'delta', date:'2021-02-22', lane:'cmp', src:'2102.11174',
 ...
 costs:'Read-then-write is inherently sequential, which is what a GPU hates — so it stayed impractical to train at scale for three years, until #16. And capacity is still fixed: correction manages the budget well, it does not enlarge it.',
 ...
 tier:'A',
 verified:'arXiv v1 metadata.',
 note:'<strong>Disputed priority, kept visible on the timeline as an arc.</strong> This is the actual origin of the delta-rule correction — it predates the paper everyone calls "the DeltaNet paper" (#16) by over three years, and even predates RoPE.'},
...
{n:9, id:'rope', date:'2021-04-20', lane:'pos', src:'2104.09864',
 ...
 costs:'Rotation frequencies are fixed at training time. Push past the trained length and the low-frequency dimensions produce angles the model has never seen — defined everywhere, competent only where it trained. That single limitation generates #12, #12b, #13 and arguably #20.',
...
{n:11, id:'gqa', date:'2023-05-22', lane:'mem', src:'2305.13245',
...
{n:12, id:'ntk', date:'2023-06-29', lane:'pos', src:'reddit',
...
{n:'12b', id:'ntk-parts', date:'2023-07-07', lane:'pos', src:'github',
 ...
 note:'Split out from #12 during the date audit. <strong>YaRN builds on this, not on plain NTK-Aware</strong> — 2309.00071 §3.2 says <em>"a variant of the resulting method was released under the name \'NTK-by-parts\' interpolation"</em>. The lineage arrow points here.'},

{n:13, id:'yarn', date:'2023-08-31', lane:'pos', src:'2309.00071',
 ...
 tier:'A',
 verified:'arXiv v1 metadata. Its own bibliography is also what established that #12 and #12b have no papers, and what separated the two.'},

{n:14, id:'sinks', date:'2023-09-29', lane:'mem', src:'2309.17453',
...
{n:15, id:'mla', date:'2024-05-07', lane:'mem', src:'2405.04434',
...
{n:16, id:'deltanet', date:'2024-06-10', lane:'cmp', src:'2406.06484',
...
{n:17, id:'difftransformer', date:'2024-10-07', lane:'sys', src:'2410.05258', bonus:true,
 url:'https://arxiv.org/abs/2410.05258',
 paper:'Differential Transformer',
 who:'Ye, Dong, Xia, Sun, Zhu, Huang, Wei',
 name:'Differential Transformer (DIFF)',
 one:'Subtract two softmax attention maps instead of computing one, cancelling the noise they share.',
 idea:'The bonus mechanism, and the odd one out. Every other entry above changes how much attention costs; this one changes what attention <em>is</em>, to fix a problem neither bill measures. Split each head\'s queries and keys into two groups, run two ordinary softmax attention maps in parallel, and subtract the second from the first with a small learned scalar λ: <code>DiffAttn(X) = softmax(Q₁K₁ᵀ) − λ·softmax(Q₂K₂ᵀ)</code>, applied to the same values. Whatever both maps attend to alike is noise and cancels; whatever only the first attends to survives.',
 buys:'Measurably sharper attention maps — the paper reports better long-context key-information retrieval, less hallucination in question-answering and summarisation, and more robust in-context learning against prompt-order permutation, all from the same causal Transformer stack otherwise.',
 costs:'Two attention computations instead of one, so it is not free — the paper offsets this by halving each head\'s dimension, which keeps FLOPs and KV-cache roughly on par with a standard head rather than doubling them, but the accounting is now less obvious than a single softmax. And it is a pretraining-time architectural choice, not a fix you can drop into an existing checkpoint.',
 pick:'When output faithfulness — not raw throughput or cache size — is the binding constraint: retrieval-heavy or hallucination-sensitive deployments where a less noisy attention map is worth the extra arithmetic.',
 v1:'2024-10-07T17:57:38Z',
 tier:'A',
 verified:'arXiv v1 metadata (2024-10-07T17:57:38Z) from export.arxiv.org\'s own Atom feed, <strong>plus</strong> an independent <code>pdftotext</code> read of the downloaded PDF\'s title page — checked twice because this is the bonus row and carries extra scrutiny. The PDF header itself reads "Published as a conference paper at ICLR 2025" above the arXiv stamp, confirming the same work under later peer review.',
 note:'<strong>Bonus mechanism, not covered in class — and the fix for a graded correction.</strong> An earlier draft used FlashAttention here; feedback correctly flagged that as an IO-aware <em>kernel</em>, not an attention variant — it reschedules memory traffic and returns bit-identical output, so it never changes what attention computes. Differential Transformer replaces it because it genuinely does change the computation, and it is the one entry on this whole timeline that spends <em>more</em> compute for <em>better</em> quality rather than trading quality away for a cheaper bill — the reverse of every neighbour around it.'},
...
window.LANES = {
  ...
  sys:    {name:'Signal quality',        short:'quality',  color:'var(--sys)'},
